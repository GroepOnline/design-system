...
def main() -> None:
    ...
    ap.add_argument("--out", default="dist/runtime-packages")
    args = ap.parse_args()
    ...
    if os.environ.get("GITHUB_REF_TYPE") == "tag":
        ...
        if actual != expected:
            raise SystemExit(f"tag/version mismatch: expected {expected}, got {actual}")
    subprocess.run(
        ["python3", "scripts/build-runtime-packages.py"], cwd=ROOT, check=True
    )
    subprocess.run(
        ["python3", "scripts/verify-runtime-packages.py"], cwd=ROOT, check=True
    )
    artifacts = []
    for spec in PLAN["packages"]:
        r = subprocess.run(
            ["npm", "pack", "--pack-destination", str(out), "--json"],
            cwd=ROOT / spec["path"],
            check=True,
            capture_output=True,
            text=True,
