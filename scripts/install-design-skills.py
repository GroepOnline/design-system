...
NAMES=('design-system','chef-frontend-meta','web-design-guidelines','artifact-template-design-report','communicate-clearly','locked-taste-design-run')
...
def main():
    ...
    changes=[]
    for name in NAMES:
        source=ROOT/'.agents/skills'/name/'SKILL.md';target=args.destination/name/'SKILL.md'
    ...
    for index,(source,target,content) in enumerate(changes):
        if target.is_symlink():raise SystemExit('Refusing to replace symlink adapter: '+str(target))
        item={'source':str(source),'target':str(target),'source_sha256':hashlib.sha256(source.read_bytes()).hexdigest(),'installed_sha256':hashlib.sha256(content.encode()).hexdigest()}
        if args.apply:
            args.backup.mkdir(parents=True,exist_ok=True)
            if target.exists():
                backup=args.backup/f'{index}-{target.parent.name}-SKILL.md'
                if backup.exists():raise SystemExit('Backup already exists: '+str(backup))
                backup.write_bytes(target.read_bytes());item['backup']=str(backup)
            target.parent.mkdir(parents=True,exist_ok=True);target.write_text(content)
        receipt.append(item)
    result={'applied':args.apply,'at':datetime.now(timezone.utc).isoformat(),'entries':receipt}
    if args.apply:(args.backup/'receipt.json').write_text(json.dumps(result,indent=2)+'\n')
